...
abstract class Dialogs {
  // (_defaultTitlePadding, _defaultContentPadding, _defaultMaterialActionPadding, _defaultMaterialInsetPadding)
  //  다이얼로그 내부 여백(패딩)을 통일하기 위한 EdgeInsets 상수
  ...
  // (_defaultMaterialShape) 둥근 모서리(borderRadius)를 가진 RoundedRectangleBorder로 Material 다이얼로그 모양 지정
  static final _defaultMaterialShape = RoundedRectangleBorder(
    borderRadius: BorderRadius.circular(Sizes.dialogR20),
  );

  // (_titleText, _contentText) 다이얼로그 타이틀과 본문 텍스트를 테마에 맞춰 중앙 정렬된 Text 위젯으로 생성
  ...
  static Widget _contentText(BuildContext context, String text) {
    final a = Theme.of(context);
    return Text(
      text,
      //Explicitly using style to be applied for CupertinoAlertDialog.
